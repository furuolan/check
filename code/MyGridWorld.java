...
public class MyGridWorld {
	...
	public MyGridWorld (MyMap map, double goalReward, double defaultReward, double discount){
		this.map = map;
		this.gw = new GridWorldDomain(map.getRow(),map.getCol());
		gw.setMap(map.getCoordinates());
		gw.setProbSucceedTransitionDynamics(map.getSuccessRate());
		this.domain = gw.generateDomain();
		this.initialState = GridWorldDomain.getOneAgentOneLocationState(domain);
		GridWorldDomain.setAgent(initialState, 0, 0);
		GridWorldDomain.setLocation(initialState, 0, map.getRow()-1, map.getCol()-1);
	...
	public void QLearning(String outputPath, final double qInit, final double learningRate, final double epsilon){
		...
		LearningAgentFactory qLearningFactory = new LearningAgentFactory() {
			...
			public LearningAgent generateAgent() {
				return new MyQLearning(domain, 0.99, hashingFactory, qInit, learningRate,epsilon);
			}
		};
	...
	public void initGUI(){
		Visualizer v = GridWorldVisualizer.getVisualizer(this.map.getCoordinates());
